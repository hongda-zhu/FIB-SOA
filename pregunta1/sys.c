...
void sys_exit(int error) {
    ...
    current_task->exit_error = error;
    
    // Liberar los recursos (excepto el PCB)
    free_user_pages(current_task);
    
    // Asignar los hijos al proceso idle
    list_for_each_safe(pos, next, &current_task->children) {
        child = list_head_to_task_struct(pos);
        list_del(&child->proc_anchor);
        child->parent = idle_task;
        list_add_tail(&child->proc_anchor, &idle_task->children);
    }
    
    // Desbloquear los procesos que esperan por este
    struct list_head *waiter_pos, *waiter_next;
    struct task_struct *waiter;
    
    list_for_each_safe(waiter_pos, waiter_next, &current_task->waiters) {
        waiter = list_head_to_task_struct(waiter_pos);
        list_del(waiter_pos);
        // Mover el waiter a la cola de ready
        list_add_tail(&waiter->list, &readyqueue);
    }
    
    // Marcar como zombie y añadir a la cola de zombies
    current_task->current_state = ST_ZOMBIE;
    list_add_tail(&current_task->list, &zombie_queue);
    
    // Planificar otro proces
    sched_next_rr();
}
...
int sys_waitpid(int pid, int *error) {
    ...
    // Verificar si pid es un hijo válido
    ...
    if (!found) {
        return -ECHILD;  // Error: no es un hijo válido
    }
    
    // Verificar si el hijo ya está en estado zombie
    if (child->current_state == ST_ZOMBIE) {
        // Hijo ya está zombie, obtener su error
        if (error && access_ok(VERIFY_WRITE, error, sizeof(int))) {
            copy_to_user(&child->exit_error, error, sizeof(int));
        }
        
        // Eliminar de la lista de hijos
        list_del(&child->proc_anchor);
        
        // Eliminar de la lista de zombies
        list_del(&child->list);
        
        // Liberar PCB
        int pid_to_return = child->PID;
        list_add_tail(&child->list, &freequeue);
        
        return pid_to_return;
    }
    
    // El hijo no está zombie, hay que bloquearse
    // Añadir el proceso actual a la lista de waiters del hijo
    list_add_tail(&current_task->list, &child->waiters);
    
    // Bloquear el proceso
    block();
    
    // Cuando se desbloquee, el hijo ya debería estar zombie
    // Copiar el error
    if (error && access_ok(VERIFY_WRITE, error, sizeof(int))) {
        copy_to_user(&child->exit_error, error, sizeof(int));
    }
